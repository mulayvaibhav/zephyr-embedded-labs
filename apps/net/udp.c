...
static int process_udp(struct data *data)
{
	...
	do {
		...
		vehicle_ipc_cmd_process(data->udp.recv_buffer, 
		                        received,
								111,
								&ack);
		...
		//	     &client_addr, client_addr_len);
		memset(data->udp.recv_buffer, 0x0, RECV_BUFFER_SIZE);
		memcpy(data->udp.recv_buffer, &ack, sizeof(vehicle_command_ack_wire_t));

		ret = sendto(data->udp.sock, data->udp.recv_buffer, sizeof(vehicle_command_ack_wire_t), 0,
			     &client_addr, client_addr_len);
		if (ret < 0) {
